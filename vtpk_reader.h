...
enum VectorTileHandleStatus {
  ...
  DATA_EVICTED = 2, // model data was present but was evicted from the cache and
                    // is now invalid
};
...
static unsigned char mesh_arena_level0to7_buf[MB(100)] = {0};
static unsigned char mesh_arena_level8to12_buf[MB(100)] = {0};
static unsigned char mesh_arena_level13to15_buf[MB(100)] = {0};
static unsigned char mesh_arena_level16_buf[MB(100)] = {0};

static Arena mesh_arena_level0to7 =
    (Arena){mesh_arena_level0to7_buf, sizeof(mesh_arena_level0to7_buf)};
static Arena mesh_arena_level8to12 =
    (Arena){mesh_arena_level8to12_buf, sizeof(mesh_arena_level8to12_buf)};
;
static Arena mesh_arena_level13to15 =
    (Arena){mesh_arena_level13to15_buf, sizeof(mesh_arena_level13to15_buf)};
static Arena mesh_arena_level16 =
    (Arena){mesh_arena_level16_buf, sizeof(mesh_arena_level16_buf)};
...
static S32 QuadTreeNodeFromJson(QuadTreeNodeArray *quad_tree, JsonNode *node,
                                S32 x, S32 y, S32 level) {
  assert(node != NULL);
  switch (node->type) {
  case JSON_ARRAY: {
    ASSERT(node->children.count == 4, "invalid list of children");
    const S32 child_nw = QuadTreeNodeFromJson(quad_tree, node->children.first,
                                              x * 2, y * 2, level + 1);
    const S32 child_ne = QuadTreeNodeFromJson(
        quad_tree, node->children.first->next, x * 2, (y * 2) + 1, level + 1);
    const S32 child_sw = QuadTreeNodeFromJson(
        quad_tree, node->children.last->prev, (x * 2) + 1, y * 2, level + 1);
    const S32 child_se = QuadTreeNodeFromJson(
        quad_tree, node->children.last, (x * 2) + 1, (y * 2) + 1, level + 1);
...
static S32 FileIndexFromFileName(mz_zip_archive *archive,
                                 const char *filename) {
  assert(archive != NULL);
...
static void QuadTreeFromJson(Arena *arena, VtpkFile *vtpk_file) {
  ...
  JsonNode *tree_root =
      find_key_in_children(&root, String8FromCString("index"));
  vtpk_file->root_node =
...
static void RootPropertiesFromJson(Arena *arena, VtpkFile *vtpk_file) {
  ...
  }
  {
    ...
    const JsonNode *origin_x =
        find_key_in_children(origin, String8FromCString("x"));
    const JsonNode *origin_y =
        find_key_in_children(origin, String8FromCString("y"));
...
static bool AABBContains(AABB bbox, VectorTileCoordinate coord) {
  return bbox.min_x <= coord.x && bbox.max_x >= coord.x &&
         bbox.min_y <= coord.y && bbox.max_y >= coord.y;
}

// returns all indices of nodes that intersect/touch the AABB with desired zoom
// level.
static void QuadTreeFind(const QuadTreeNodeSlice quad_tree, S32 root,
                         AABB bounding_box, S32 zoom_level,
                         S32Array *matching_nodes) {
  ...
  case INNER: {
    if (n.tile.coordinate.level < zoom_level) {
      QuadTreeFind(quad_tree, n.child_nw, bounding_box, zoom_level,
                   matching_nodes);
      QuadTreeFind(quad_tree, n.child_ne, bounding_box, zoom_level,
                   matching_nodes);
      QuadTreeFind(quad_tree, n.child_sw, bounding_box, zoom_level,
                   matching_nodes);
      QuadTreeFind(quad_tree, n.child_se, bounding_box, zoom_level,
                   matching_nodes);
...
static Arena *MeshArenaFromLevel(S32 level) {
  // TODO
}

// updates all vector tile handles that are referenced with the corresponding
// indices into the quad tree
static void VectorTileHandlesFromFile(VtpkFile *file,
                                      const S32Slice tile_indices) {
  ...
  for (S32 i = 0; i < tile_indices.count; i += 1) {
    ...
    const S32 filename_size =
        snprintf(bundle_filename, sizeof(bundle_filename),
                 "p12/tile/L%02d/R%04x/C%04x.bundle", tile->coordinate.level,
                 tile_file_row, tile_file_col);
    assert(filename_size == 31);
    const S32 file_index =
        FileIndexFromFileName(file->archive, bundle_filename);
    ...
    mz_zip_reader_extract_to_mem(file->archive, file_index, file_content,
                                 file_size, 0);
    TileBundleFileHeader *header = (TileBundleFileHeader *)file_content;
    ...
    TileIndexRecord compressed_mvt = TileIndexRecordFromIndex(
    ...
    MapboxVectorTileProtobufData mvt_protobuf = {
        .v = arena_alloc(scratch.arena, uncompressed_tile_size),
        .size = uncompressed_tile_size};
    U64 decompressed_size = tinfl_decompress_mem_to_mem(
    ...
    Arena *mesh_arena = MeshArenaFromLevel(tile->coordinate.level);
    tile->gpu_data = ParseMapboxVectorTile(mesh_arena, mvt_protobuf);
