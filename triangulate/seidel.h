...
static S32 InitQueryStructure(QueryNodeArray *query_structure, Coord2Slice vertices,
                              TrapezoidArray *trapezoids, SegmentArray *segments,
                              S32 initial_segment);
static void ConstructTrapezoidation(QueryNodeArray *query_structure, Coord2Slice vertices,
                                    SegmentArray *segments, TrapezoidArray *trapezoids,
                                    S32Array *permutation);
static void AddSegment(QueryNodeArray *query_structure, Coord2Slice vertices,
                       SegmentSlice segments, TrapezoidArray *trapezoids, S32 segment);
static void MergeTrapezoids(QueryNodeSlice query_structure, TrapezoidArray *trapezoids,
                            S32 segment, S32 first_trapezoid, S32 last_trapezoid,
                            MergeSide side);
static S32 TrapezoidIndexFromVertex(QueryNodeSlice query_structure, SegmentSlice segments,
                                    Coord2Slice vertices, S32 vertex,
                                    S32 segment_endpoint, S32 root);
...
static void TriangulateMonotonePolygons(VertexChainSlice vertex_chains,
                                        MonotoneChainSlice monotone_polygon_chains,
                                        S32Slice monotone_chain_start_vertex,
                                        TriangleArray *op);
static void TriangulateSingleMonotonePolygon(VertexChainSlice vertex_chains,
                                             MonotoneChainSlice monotone_chains,
                                             S32 y_max_index, MonotoneBaseSide side,
...
static bool VertexLeftOfSegment(Coord2Slice vertices, SegmentSlice segments, S32 segment,
                                S32 vertex);
...
static void ValidateQueryTrapezoidSegmentStructures(QueryNodeSlice qs, TrapezoidSlice ts,
...
static void ValidateMonotonePolygon(VertexChainSlice vertex_chains, MonotoneChainSlice mc,
                                    S32 y_max_index, MonotoneBaseSide side);
#endif
...
void TessalatePolygon(TriangleArray *triangles, const Coord2Slice coords,
    ...
    ASSERT(coords.count == sum,
           "coordinate count: %d and countour sizes: %d do not match", coords.count, sum);
    ...
    QueryNodeArray query_structure = QueryNodeArrayNew(scratch.arena, 12 * coords.count);
    SegmentArray segments = SegmentArrayNew(scratch.arena, coords.count);
    TrapezoidArray trapezoids = TrapezoidArrayNew(scratch.arena, 6 * coords.count);
    ...
    VertexChainArray vertex_chains = VertexChainArrayNew(scratch.arena, coords.count);
    S32Array monotone_chain_start_vertex = S32ArrayNew(scratch.arena, coords.count);
    ...
    MonotonateTrapezoids(
        &vertex_chains, &monotone_polygon_chains, TrapezoidSliceFromArray(&trapezoids),
        SegmentSliceFromArray(&segments), coords, S32SliceFromArray(&visited_trapezoids),
        &monotone_chain_start_vertex);
    TriangulateMonotonePolygons(VertexChainSliceFromArray(&vertex_chains),
    ...
    for (S32 i = triangle_index_min; i < triangles->count; i += 1) {
        ...
        ASSERT(cross >= 0.f,
               ...
               "[(%f, %f), (%f, %f), (%f, %f)]",
               t.a, t.b, t.c, cross, coords.v[t.a].x, coords.v[t.a].y, coords.v[t.b].x,
...
#ifdef DEBUG_GRAPHICAL
    Camera2D camera = {.offset = (Vector2){(float)1920 / 2.0f, (float)1080 / 2.0f},
                       .rotation = 0.0f,
                       .zoom = 4368.0f,
    ...
    for (S32 i = 0; i < monotone_chain_start_vertex.count; i++) {
        ...
        BeginDrawing();
        {
            Vector2 a = GetWorldToScreen2D(
                Vector2FromCoord2YFlipped(vertex_chains.v[first_vertex].pt), camera);
            Vector2 b = GetWorldToScreen2D(
                Vector2FromCoord2YFlipped(
        ...
        while (monotone_polygon_chains.v[p].vertex != first_vertex) {
            ...
            Vector2 b = GetWorldToScreen2D(
                Vector2FromCoord2YFlipped(
                    vertex_chains
                        .v[monotone_polygon_chains.v[monotone_polygon_chains.v[p].next]
                               .vertex]
                        .pt),
                camera);
            BeginDrawing();
        ...
        if (vertex_count == 3) {
            TriangleArrayPush(
                ...
                (Triangle){
                    ...
                    monotone_polygon_chains.v[monotone_polygon_chains.v[p].next].vertex,
                    monotone_polygon_chains.v[monotone_polygon_chains.v[p].prev].vertex});
        } else {
            v = monotone_polygon_chains.v[monotone_polygon_chains.v[posmax].next].vertex;
...
static void TriangulateSingleMonotonePolygon(VertexChainSlice vertex_chains,
    ...
    ASSERT(CROSS(vertex_chains.v[v1].pt, vertex_chains.v[v2].pt, vertex_chains.v[v].pt) >
               0,
           "Triangle [%d, %d, %d] not in counter clockwise order\n", v1, v2, v)
    TriangleArrayPush(triangles, (Triangle){v1, v2, v});
...
static void TraversePolygon(TraversalInfo *TI, S32 current_monotone,
                            ...
                            TraversalDirection dir) {
#ifdef DEBUG
    ValidateMonotoneChains(MonotoneChainSliceFromArray(TI->monotone_polygon_chains));
#endif
            ...
            if (Coord2EqualTo(t.max_y,
                ...
                if (dir == DOWN) {
                    ...
                    TraversePolygon(TI, new_monotone, t.up1, current_trapezoid, UP);
                           ...
                           t.max_y, TI->vertices.v[TI->segments.v[t.right_segment].v1]) &&
                       Coord2EqualTo(t.min_y,
                                     TI->vertices.v[TI->segments.v[t.left_segment].v1])) {
...
//    (v0)- - - - - - - - - - - - -(v1)
//    / \ <- minimize this angle     |
static void NextVertexIndexForMonotoneChain(VertexChainSlice vertex_chains, S32 v0,
                                            S32 v1, S32 *ip, S32 *iq) {
    ASSERT(v0 != 0, "v0 is 0\n")
    ASSERT(v1 != 0, "v1 is 0\n")
    S32 tp = 0, tq = 0;
...
static S32 SplitPolygonByDiagonal(VertexChainSlice vertex_chains,
                                  ...
                                  S32 current_monotone_polygon, S32 v0, S32 v1) {
#ifdef DEBUG
    ValidateMonotoneAndVertexChains(vertex_chains,
                                    MonotoneChainSliceFromArray(monotone_polygon_chains),
                                    S32SliceFromArray(monotone_chain_start_vertex));
#endif
    S32 ip, iq;
    ...
    // for the new list
    const S32 i = MonotoneChainArrayPush(monotone_polygon_chains, (MonotoneChain){0});
    const S32 j = MonotoneChainArrayPush(monotone_polygon_chains, (MonotoneChain){0});
...
#ifdef DEBUG
    fprintf(stderr, "make_poly: current_monotone_polygon = %d, (v0, v1) = (%d, %d)\n",
    ...
    if (polygon_length_p != polygon_length_i) {
        ASSERT(polygon_length_p + polygon_length_i == polygon_length_prev + 2,
               ...
               "!= %d",
               current_monotone_polygon, new_monotone, polygon_length_p, polygon_length_i,
               polygon_length_prev + 2)
    }
#endif
    return new_monotone;
}
...
static void ConstructTrapezoidation(QueryNodeArray *query_structure, Coord2Slice vertices,
    ...
    S32 query_root = InitQueryStructure(query_structure, vertices, trapezoids, segments,
                                        RandomSegment(permutation));
    ...
    ValidateQueryTrapezoidSegmentStructures(QueryNodeSliceFromArray(query_structure),
                                            ...
                                            SegmentSliceFromArray(segments), vertices);
#endif

    S32 log_star_segment_count = MathLogStar(segment_count);
    for (S32 h = 1; h <= log_star_segment_count; h++) {
        for (S32 i = MathN(segment_count, h - 1) + 1; i <= MathN(segment_count, h); i++) {
#ifdef DEBUG
            ...
            for (S32 s = 1; s < segments->count; s += 1) {
                ...
                if (seg.is_inserted) {
                    Vector2 a = GetWorldToScreen2D(
                        Vector2FromCoord2YFlipped(vertices.v[seg.v0]), camera);
                    Vector2 b = GetWorldToScreen2D(
                        Vector2FromCoord2YFlipped(vertices.v[seg.v1]), camera);
    ...
    for (S32 i = MathN(segment_count, log_star_segment_count) + 1; i <= segment_count;
         i++) {
#ifdef DEBUG
        ValidateQueryTrapezoidSegmentStructures(
            ...
            SegmentSliceFromArray(segments), vertices);
#endif
        AddSegment(query_structure, vertices, SegmentSliceFromArray(segments), trapezoids,
                   RandomSegment(permutation));
#ifdef DEBUG
        ValidateQueryTrapezoidSegmentStructures(
            QueryNodeSliceFromArray(query_structure), TrapezoidSliceFromArray(trapezoids),
...
static S32 AddVertex(QueryNodeArray *query_structure, Coord2Slice vertices,
                     SegmentSlice segments, TrapezoidArray *trapezoids, S32 v0, S32 v1,
                     S32 v0_root, bool first_vertex) {

    const S32 old_upper_trapezoid = TrapezoidIndexFromVertex(
        QueryNodeSliceFromArray(query_structure), segments, vertices, v0, v1, v0_root);
    ...
    trapezoids->d[old_upper_trapezoid].min_y = trapezoids->d[new_lower_trapezoid].max_y =
        vertices.v[v0];
...
static void AddSegment(QueryNodeArray *query_structure, Coord2Slice vertices,
    ...
    if (!v0_inserted) {
        top_trapezoid = AddVertex(query_structure, vertices, segments, trapezoids, s.v0,
                                  s.v1, s.root0, true);
    } else {
        top_trapezoid = TrapezoidIndexFromVertex(QueryNodeSliceFromArray(query_structure),
                                                 segments, vertices, s.v0, s.v1, s.root0);
    }
    if (!v1_inserted) {
        bottom_trapezoid = AddVertex(query_structure, vertices, segments, trapezoids,
                                     s.v1, s.v0, s.root1, false);
    } else {
        bottom_trapezoid =
            TrapezoidIndexFromVertex(QueryNodeSliceFromArray(query_structure), segments,
                                     vertices, s.v1, s.v0, s.root1);
        tribot = true;
    }
#ifdef DEBUG
    ValidateQueryTrapezoidSegmentStructures(QueryNodeSliceFromArray(query_structure),
                                            TrapezoidSliceFromArray(trapezoids), segments,
                                            vertices);
#endif
    ...
    while (t && Coord2GreaterThanEqualTo(trapezoids->d[t].min_y,
        ...
        S32 new_trapezoid = TrapezoidArrayPush(trapezoids, (Trapezoid){.is_valid = true});
        // left trapezoid sink
        ...
        S32 right_sink_node = QueryNodeArrayPush(
            query_structure,
            (QueryNode){.node_type = SINK, .trapezoid = new_trapezoid, .parent = sk});
        ...
        if (Coord2EqualTo(trapezoids->d[t].min_y, trapezoids->d[bottom_trapezoid].min_y))
        ...
        if (!(trapezoids->d[t].up0 || trapezoids->d[t].up1)) {
            ERROR_MSG("AddSegment: error no upper trapezoid of the one to split exists, "
        ...
        if (trapezoids->d[t].down0 && !trapezoids->d[t].down1) {
            ...
            if (FP_EQUAL(trapezoids->d[t].min_y.y, vertices.v[s.v0].y)) {
                ...
                F64 yt =
                    (y0 - vertices.v[s.v0].y) / (vertices.v[s.v1].y - vertices.v[s.v0].y);
                tmppt.x =
        ...
        trapezoids->d[t_sav].right_segment = trapezoids->d[tn_sav].left_segment = segment;
    }
...
static void MergeTrapezoids(QueryNodeSlice query_structure, TrapezoidArray *trapezoids,
    ...
    while (t && Coord2GreaterThanEqualTo(trapezoids->d[t].min_y,
        ...
        if (next_trapezoid_shares_segment &&
            trapezoids->d[t].left_segment == trapezoids->d[next_trapezoid].left_segment &&
...
static bool NodeIsReachable(const QueryNodeSlice qs, S32 root, S32 node) {
    ...
    return NodeIsReachable(qs, q.left, node) || NodeIsReachable(qs, q.right, node);
}
...
static void ValidateTrapezoidStructure(const TrapezoidSlice ts) {
    ...
    ASSERT(!ts.v[0].down1, "Trapezoids[0].down1 is not zeroed, was %d instead",
           ts.v[0].down1)
    ASSERT(!ts.v[0].up0, "Trapezoids[0].up0 is not zeroed, was %d instead", ts.v[0].up0)
    ASSERT(!ts.v[0].up1, "Trapezoids[0].up1 is not zeroed, was %d instead", ts.v[0].up1)
    for (S32 i = 1; i < ts.count; i += 1) {
        ...
        if (t.left_segment || t.right_segment) {
            ...
            ASSERT(t.left_segment != t.right_segment,
                   "Trapezoid %d has identical right/left segment: %d", i, t.left_segment)
        }
        ...
        if (t.up0 && t.up1) {
            ASSERT(!(ts.v[t.up0].down0 && ts.v[t.up0].down1),
                   "Trapezoid %d can not have two lower neighbors up0 of %d", t.up0, i)
            ASSERT(!(ts.v[t.up1].down0 && ts.v[t.up1].down1),
                   "Trapezoid %d can not have two lower neighbors up1 of %d", t.up1, i)
        }
        if (t.down0 && t.down1) {
...
static void ValidateQueryTrapezoidSegmentStructures(const QueryNodeSlice qs,
                                                    const TrapezoidSlice ts,
                                                    const SegmentSlice segments,
                                                    const Coord2Slice vertices) {
    ...
    for (S32 i = 1; i < ts.count; i += 1) {
        ...
        if (t.up0 && t.up1 && ts.v[t.up0].right_segment && ts.v[t.up1].right_segment) {
            ASSERT(!VertexLeftOfSegment(vertices, segments, ts.v[t.up0].right_segment,
                                        segments.v[ts.v[t.up1].right_segment].v0) ||
                       !VertexLeftOfSegment(vertices, segments, ts.v[t.up0].right_segment,
...
static void ValidateMonotoneChains(const MonotoneChainSlice chains) {
    for (S32 i = 1; i < chains.count; i += 1) {
        ...
        ASSERT(i == chains.v[i_prev].next, "Prev and next of %d and %d disagree", i_prev,
               i)
        ASSERT(i == chains.v[i_next].prev, "Next and prev of %d and %d disagree", i_next,
               i)
    }
}
static void ValidateMonotoneAndVertexChains(const VertexChainSlice vc,
    ...
    for (S32 i = found_vertices.count - 1; i > 0; i -= 1) {
        ...
        if (found_inside) {
            ASSERT(found_vertices.d[i],
                   "Vertex %d (%f, %f), not found in monotone chains", i, vc.v[i].pt.x,
                   vc.v[i].pt.y);
        }
    }
    temp_arena_memory_end(scratch);
...
static void ValidateMonotonePolygon(VertexChainSlice vertex_chains, MonotoneChainSlice mc,
    ...
    while (current_vertex != y_min_index) {
        ASSERT(Coord2LessThan(vertex_chains.v[mc.v[current_vertex].vertex].pt, y_current),
               "Polygon is not a monotone mountain");
        ASSERT(Coord2GreaterThanEqualTo(vertex_chains.v[mc.v[current_vertex].vertex].pt,
                                        y_min),
