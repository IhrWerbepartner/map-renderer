...
static VectorTileHandleSlice MeshesFromBoundingBox(VtpkFile *vtpk_file,
                                                   AABB bounding_box,
  ...
  ScratchArena() {
    ...
    QuadTreeFind(QuadTreeNodeSliceFromArray(&vtpk_file->quad_tree),
                 vtpk_file->root_node, bounding_box, zoom_level,
                 &visible_tiles);
    ...
    const S32Slice missing_tile_indices =
        RemoveAllCachedTileIndices(visible_tiles, draw_cache->back_buffer);
  ...
  Swap(VectorTileHandleArray, draw_cache->front_buffer,
       draw_cache->back_buffer);
...
void VtpkDisplayFile(char *filename, Screen screen) {
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    BeginMode2D(camera);
    {
      VectorTileHandleSlice models = MeshesFromBoundingBox(
          vtpk_file, bounding_box, &draw_cache, ZoomLevelFromCamera(camera));
      for (S32 i = 0; i < models.count; i += 1) {
        assert(models.v[i].status == DATA_PRESENT);
        DrawModel(*models.v[i].mesh, (Vector3){0}, 1.f, RED);
      }
    }
    EndMode2D();
    EndDrawing();
